...
import dev.emi.trinkets.TrinketsConfig;
...
public class SlotLoader extends SinglePreparationResourceReloader<Map<String, GroupData>> implements IdentifiableResourceReloadListener {
	...
	protected void apply(Map<String, GroupData> loader, ResourceManager manager, Profiler profiler) {
		this.slots = loader;
		TrinketsConfig.load();
		for (var groupEntry : this.slots.entrySet()) {
			String groupName = groupEntry.getKey();
			GroupData group = groupEntry.getValue();
			for (var slotEntry : group.slots.entrySet()) {
				String slotName = slotEntry.getKey();
				int override = TrinketsConfig.getOverrideAmount(groupName, slotName);
				if (override >= 0) {
					slotEntry.getValue().overrideAmount(override);
				}
			}
		}
	}
	...
	static class SlotData {
		...
		void overrideAmount(int amount) {
			if (amount >= 0) {
				this.amount = amount;
			}
		}
